...
ShootRay(double u, double v)
{
	...
	if(u < 0.0 || u > 1.0 || v < 0.0 || v > 1.0) {
		...
		// Do lens calculations in order to achieve
		// depth of field!
		if(focusDistance > 0.0 && lensRadius > 0.0)
		{
			double cosAngle = DOT(vz, ray.direction);
			// Get focus point
			Point focusOnPoint;
			TIMES(focusOnPoint, ray.direction, (focusDistance / cosAngle));
			PLUS(focusOnPoint, focusOnPoint, camera);
			Point randomPoint;
			randomPoint.v[0] = (2.0*frandom() - 1.0) * lensRadius;
			randomPoint.v[1] = (2.0*frandom() - 1.0) * lensRadius;
			randomPoint.v[2] = 0.0;
			//final results			
			PLUS(ray.origin, randomPoint, camera);
			MINUS(ray.direction, focusOnPoint, ray.origin);
			(void) Normalize(&ray.direction);
		}
	}
